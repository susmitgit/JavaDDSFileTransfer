...
public class FileFragmentTypeSupport extends TypeSupportImpl {
    // -----------------------------------------------------------------------
    // Private Fields
    // -----------------------------------------------------------------------

    private static final String TYPE_NAME = "com::rti::examples::JavaDDSFileTransfer::FileFragment";

    private static final char[] PLUGIN_VERSION = {2, 0, 0, 0};     
    private static final FileFragmentTypeSupport _singleton
    = new FileFragmentTypeSupport();
    ...
    */
    public static FileFragmentTypeSupport get_instance() {
        return _singleton;
    }

    public static FileFragmentTypeSupport getInstance() {
        return get_instance();
    }

    public static TypeCode getTypeCode(){
        return FileFragmentTypeCode.VALUE;
    }
    ...
    * <code>this</code> is not a <code>FileFragment</code>
    * type.
    */
    ...
    public long get_serialized_sample_max_size(Object endpoint_data,boolean include_encapsulation,short encapsulation_id,long currentAlignment) {
        ...
        currentAlignment += CdrPrimitiveType.getStringMaxSizeSerialized(currentAlignment, (((rti.examples.JavaDDSFileTransfer.FILE_NAME_LENGTH_MAX.VALUE)))+1);
        currentAlignment += CdrPrimitiveType.INT.getMaxSizeSerialized(currentAlignment );
        currentAlignment += CdrPrimitiveType.INT.getMaxSizeSerialized(currentAlignment );
        currentAlignment += CdrPrimitiveType.INT.getMaxSizeSerialized(currentAlignment );
        currentAlignment += CdrPrimitiveType.BYTE.getSequenceMaxSizeSerialized(currentAlignment, ((rti.examples.JavaDDSFileTransfer.FILE_FRAGMENT_SIZE_MAX.VALUE)));
        if (include_encapsulation) {
        ...
        Object sample) 
    {
        ...
        currentAlignment  +=  CdrPrimitiveType.getStringSerializedSize(currentAlignment , typedSrc.file_name );
        currentAlignment  +=  CdrPrimitiveType.INT.getMaxSizeSerialized(currentAlignment);
        currentAlignment  +=  CdrPrimitiveType.INT.getMaxSizeSerialized(currentAlignment);
        currentAlignment  +=  CdrPrimitiveType.INT.getMaxSizeSerialized(currentAlignment);
        currentAlignment  +=   CdrPrimitiveType.BYTE.getSequenceSerializedSize(
            currentAlignment , typedSrc.frag );      
    ...
    short encapsulation_id, boolean serialize_sample, Object endpoint_plugin_qos) {
        ...
        if(serialize_sample) {
            ...
            dst.writeString(typedSrc.file_name,((rti.examples.JavaDDSFileTransfer.FILE_NAME_LENGTH_MAX.VALUE)));

            dst.writeInt(typedSrc.file_size);

            dst.writeInt(typedSrc.frag_num);

            dst.writeInt(typedSrc.frag_total);

            dst.writeByteSeq(typedSrc.frag,((rti.examples.JavaDDSFileTransfer.FILE_FRAGMENT_SIZE_MAX.VALUE)));
        }
        ...
        Object endpoint_plugin_qos) 
    {
        ...
        if(deserialize_sample) {
            ...
            typedDst.clear();      
            try{
                typedDst.file_name = src.readString(((rti.examples.JavaDDSFileTransfer.FILE_NAME_LENGTH_MAX.VALUE)));
                typedDst.file_size = src.readInt();
                typedDst.frag_num = src.readInt();
                typedDst.frag_total = src.readInt();
                src.readByteSequence(typedDst.frag, ((rti.examples.JavaDDSFileTransfer.FILE_FRAGMENT_SIZE_MAX.VALUE)));
    ...
    int mask) {
        return new FileFragmentDataWriter (native_writer, listener, mask, this);            
    }
    ...
    int mask) {
        ...
        return new FileFragmentDataReader(native_reader, listener, mask, this);   
    ...
    protected FileFragmentTypeSupport() {

        /* If the user data type supports keys, then the second argument
        to the constructor below should be true.  Otherwise it should
        be false. */        

        super(TYPE_NAME, false,FileFragmentTypeCode.VALUE,FileFragment.class,TypeSupportType.TST_STRUCT, PLUGIN_VERSION);

    }

    protected FileFragmentTypeSupport (boolean enableKeySupport) {

        super(TYPE_NAME, enableKeySupport,FileFragmentTypeCode.VALUE,FileFragment.class,TypeSupportType.TST_STRUCT, PLUGIN_VERSION);
    }
}
